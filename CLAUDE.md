...
- **Buildings dataset is OSM, not a government registry.** No Malaysian
  ...
  `scripts/enrich-names.mjs` reverse-geocodes the unnamed / bare-block
  buildings against Google (`result_type=premise`) and now ~564/1,119 have
  a real complex name (`name_source` column: `osm` | `google` | `block`);
  `block` column holds "Block A" / "Tower B" labels. `buildingLabel()` in
  `src/index.js` renders "<complex> - Block A" / "Block A - 12-storey -
  City" / "12-storey - City". Re-run: `node scripts/enrich-names.mjs`
  (needs `GOOGLE_MAPS_API_KEY` or web/.env), then apply
  `buildings_names_seed.sql`. The `names.cache.json` caches geocodes. The level filter
...
buildings_names_schema.sql  block + name_source columns            } generated by
buildings_names_seed.sql    real complex names (Google reverse geocode) } enrich-names.mjs
scripts/enrich-sources.mjs  Overpass pull -> building_sources + segments (run locally)
scripts/enrich-names.mjs    Google reverse-geocode -> real building names (run locally)
scripts/buildings.json      dump of `buildings` for the enrich scripts (regenerate from D1)
